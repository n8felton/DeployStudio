...
then
  NETBOOT_SERVER_IP="\<string\>-server=${NETBOOT_SERVER_IP}\<\/string\>"
fi

if [ -n "${NETBOOT_SET_FOLDER_NAME}" ]
then
  NETBOOT_SET_FOLDER_NAME="\<string\>-nbi=${NETBOOT_SET_FOLDER_NAME}\<\/string\>"
fi
...
then
  ...
  then
    ...
    sed -e s/__HOUR__/${HOUR}/g \
        ...
        -e s/__NETBOOT_SERVER_IP__/"${NETBOOT_SERVER_IP}"/g \
        -e s/__NETBOOT_SET_FOLDER_NAME__/"${NETBOOT_SET_FOLDER_NAME}"/g \
        -e s/__FORCE__/"${FORCE}"/g \
		"${SCRIPT_PATH}"/ds_netboot_restart/com.deploystudio.netboot_restart_daily.plist > "${VOLUME_PATH}"/Library/LaunchDaemons/com.deploystudio.netboot_restart.plist 	
    ...
    printf "\n${MINUTE}\t${HOUR}\t*\t*\t*\troot\t/etc/deploystudio/bin/ds_netboot_restart.sh" >> "${VOLUME_PATH}"/etc/crontab
...
then
  ...
  then
    ...
    sed -e s/__HOUR__/${HOUR}/g \
        -e s/__MINUTE__/${MINUTE}/g \
        -e s/__WEEKDAY__/${DAY}/g \
        -e s/__NETBOOT_SERVER_IP__/"${NETBOOT_SERVER_IP}"/g \
        -e s/__NETBOOT_SET_FOLDER_NAME__/"${NETBOOT_SET_FOLDER_NAME}"/g \
        -e s/__FORCE__/"${FORCE}"/g \
		"${SCRIPT_PATH}"/ds_netboot_restart/com.deploystudio.netboot_restart_weekly.plist > "${VOLUME_PATH}"/Library/LaunchDaemons/com.deploystudio.netboot_restart.plist
	chmod 644 "${VOLUME_PATH}"/Library/LaunchDaemons/com.deploystudio.netboot_restart.plist
	chown root:wheel "${VOLUME_PATH}"/Library/LaunchDaemons/com.deploystudio.netboot_restart.plist
  else
    echo "configuring crontab to netboot the computer at ${HOUR}:${MINUTE} every week on ${DAY}"
    printf "\n${MINUTE}\t${HOUR}\t*\t*\t${DAY}\troot\t/etc/deploystudio/bin/ds_netboot_restart.sh" >> "${VOLUME_PATH}"/etc/crontab 
    sed '/^[[:space:]]*$/d' "${VOLUME_PATH}"/etc/crontab > "${VOLUME_PATH}"/etc/crontab2
    mv -f "${VOLUME_PATH}"/etc/crontab2 "${VOLUME_PATH}"/etc/crontab
  fi
...
then
  ...
  then
    ...
    sed -e s/__HOUR__/${HOUR}/g \
        ...
        -e s/__NETBOOT_SET_FOLDER_NAME__/"${NETBOOT_SET_FOLDER_NAME}"/g \
        -e s/__FORCE__/"${FORCE}"/g \
		"${SCRIPT_PATH}"/ds_netboot_restart/com.deploystudio.netboot_restart_monthly.plist > "${VOLUME_PATH}"/Library/LaunchDaemons/com.deploystudio.netboot_restart.plist
	chmod 644 "${VOLUME_PATH}"/Library/LaunchDaemons/com.deploystudio.netboot_restart.plist
	chown root:wheel "${VOLUME_PATH}"/Library/LaunchDaemons/com.deploystudio.netboot_restart.plist
  else
...
cp "${SCRIPT_PATH}"/ds_netboot_restart/ds_netboot_restart.sh "${VOLUME_PATH}"/etc/deploystudio/bin/ds_netboot_restart.sh
chmod 700 "${VOLUME_PATH}"/etc/deploystudio/bin/ds_netboot_restart.sh
chown root:wheel "${VOLUME_PATH}"/etc/deploystudio/bin/ds_netboot_restart.sh
